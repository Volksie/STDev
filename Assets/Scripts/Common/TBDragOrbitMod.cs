...
public class TBDragOrbitMod : MonoBehaviour
{
    public enum PanMode
    {
        Disabled,
	...
	/// <summary>
    /// Keep pitch angle value between minYaw and maxYaw?
    /// </summary>
    ...
    public float IdealYaw
    {
        get { return idealYaw; }
        //set { idealYaw = value; }
		set { idealYaw = clampYawAngle ? ClampYawAngle( value, minYaw, maxYaw ) : value; }
    }
    ...
    public float IdealPitch
    {
        ...
        set { idealPitch = clampPitchAngle ? ClampPitchAngle( value, minPitch, maxPitch ) : value; }
    }
    ...
    void Apply()
    {
        ...
        transform.position = ( targetPosition.position + panOffset ) - distance * transform.forward;
    }
    ...
    static float ClampPitchAngle( float angle, float min, float max )
    {
        if( angle < -360 )
            angle += 360;
	...
	static float ClampYawAngle( float angle, float min, float max )
    {
        if( angle < -360 )
            angle += 360;
	...
	public void SetTargetSmooth(Transform newTarget)
	{
		if (targetPosition)
		{
			panOffset = targetPosition.position - newTarget.position;
		}
