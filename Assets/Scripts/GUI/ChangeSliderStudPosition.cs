...
public class ChangeSliderStudPosition : MonoBehaviour
{
	
	public UISlider studPositionSlider;
	...
	void OnEnable () {
        ...
        Debug.Log("Offset " + AppRoot.Scene.SelectedWallElement.Offset);

        studPositionSlider.sliderValue = (AppRoot.Scene.SelectedWallElement.Offset - mMinValue) / (mMaxValue - mMinValue);
        //rowHeightSlider.sliderValue = (float)AppRoot.Scene.SelectedWall.Length / ( mMaxValue - mMinValue);
        OnSliderChange(studPositionSlider.sliderValue);
